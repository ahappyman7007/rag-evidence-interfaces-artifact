...
\texttt{https://anonymous.4open.science/r/}\linebreak
\texttt{rag-evidence-interfaces-artifact-AA35/}. It includes the code,
configuration files, prompt templates, aggregate tables, plotting scripts, and
paper source needed to reproduce the reported analyses. The experiments use
public third-party datasets and model checkpoints, which should be obtained
from their original providers under their respective licenses. The artifact
does not include credentials, local paths, private logs, non-redistributable
model checkpoints, or raw third-party dataset copies. For restricted resources,
it provides derived aggregate tables and reconstruction scripts rather than the
original dataset text. The artifact README separates quick CPU smoke tests,
aggregate-result rebuilding, and full GPU reruns from public resources.
